...
export default function Dashboard() {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-4">
        <div className="w-full max-w-sm bg-[var(--color-surface)] border border-[var(--color-border)] rounded-sm p-8">
          ...
          <p className="text-muted text-sm text-center mb-6">Enter password to continue.</p>
          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <input
              ...
              className="w-full px-4 py-3 bg-[var(--color-surface-light)] border border-[var(--color-border)] rounded-sm text-secondary placeholder-muted/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
              autoFocus
            ...
            <button
              type="submit"
              className="w-full bg-primary text-[var(--color-bg)] font-bold py-3 rounded-sm hover:bg-[var(--color-primary-hover)] transition-colors uppercase tracking-wider"
            >
              Enter
            </button>
          </form>
  ...
  return (
    <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-[1600px] mx-auto w-full">
      <div className="mb-12">
        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-primary mb-2">Admin</p>
        <h1 className="font-serif text-4xl font-bold text-secondary mb-2 tracking-wide">
          Dashboard
        </h1>
        <div className="w-12 h-px bg-primary mt-4 mb-2" />
        <p className="text-muted">Manage your portfolio and view leads.</p>
      </div>
      ...
      <div className="flex space-x-4 mb-8 border-b border-[var(--color-border)] pb-4">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-sm font-bold uppercase tracking-wider text-sm transition-colors ${
            activeTab === 'media' 
              ? 'bg-primary text-[var(--color-bg)]' 
              : 'bg-[var(--color-surface)] text-muted border border-[var(--color-border)] hover:border-primary/50 hover:text-primary'
          }`}
        >
      ...
      {error && (
        <div className="mb-8 p-4 bg-accent/10 border border-accent/30 rounded-sm flex items-start gap-3 text-accent">
          <AlertCircle className="w-5 h-5 shrink-0 mt-0.5" />
          <div>
            <p className="font-bold">Error</p>
            <p className="text-sm">{error}</p>
            {(error.includes('credentials') || error.includes('GitHub configuration')) && (
              <p className="text-xs mt-2 text-accent/80 font-medium">
      ...
      {mediaHint && activeTab === 'media' && !error && (
        <div className="mb-8 p-4 bg-primary/5 border border-primary/20 rounded-sm text-primary text-sm">
          <p className="font-medium">Media not connected</p>
          <p className="mt-1 text-primary/70">{mediaHint}</p>
        </div>
      )}
        ...
        <div className="space-y-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-[var(--color-surface)] p-6 rounded-sm border border-[var(--color-border)]">
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-sm text-sm font-bold transition-colors ${
                    category === cat.id
                      ? 'bg-primary text-[var(--color-bg)]'
                      : 'bg-[var(--color-surface-light)] text-muted border border-[var(--color-border)] hover:border-primary/50 hover:text-primary'
                  }`}
                >
                  {cat.name}
                </button>
            ...
            <div className="relative">
              ...
              <label
                htmlFor="file-upload"
                className={`flex items-center gap-2 px-6 py-3 rounded-sm font-bold uppercase tracking-wider cursor-pointer transition-colors text-sm ${
                  uploading
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-muted bg-[var(--color-surface)] rounded-sm border border-[var(--color-border)]">
                ...
                return (
                  <div key={item.sha} className="group relative rounded-sm overflow-hidden border border-[var(--color-border)] bg-[var(--color-surface)]">
                    ...
                    <div className="absolute inset-0 bg-[var(--color-bg)]/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center p-4">
                      <p className="text-xs text-muted truncate w-full text-center mb-4">{item.name}</p>
                      <button
                        onClick={() => handleDelete(item.path, item.sha)}
                        className="p-2 bg-accent text-white hover:bg-accent/80 rounded-sm transition-colors"
                        title="Delete file"
                      >
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-muted">
                <thead className="text-xs text-primary uppercase bg-[var(--color-surface-light)] border-b border-[var(--color-border)] font-bold tracking-wider">
                  <tr>
                  ...
                  {contacts.map((contact) => (
                    <tr key={contact.id} className="border-b border-[var(--color-border)] hover:bg-[var(--color-surface-light)]">
                      ...
                      <td className="px-6 py-4">
                        <div className="font-medium text-secondary">{contact.email}</div>
                        <div className="text-muted/60">{contact.phone}</div>
                      </td>
